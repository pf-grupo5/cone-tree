...
struct AABB
{
    ...
    [[nodiscard]] std::pair<float, float> intersection_time(const ray& ray, float min_time, float max_time) const
    {
        ...
        float enter_time = t_1 >= t_0 && t_1 >= min_time && t_0 < max_time ? t_0 : 1e30f;
        float exit_time = t_1 >= t_0 && t_1 >= min_time && t_1 < max_time ? t_1 : 1e30f;
        return {enter_time, exit_time};
    }
    [[nodiscard]] auto operator[](int i) const noexcept -> const float& { return i > 2 ? max[i - 2] : min[i]; }
};
